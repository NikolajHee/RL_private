...
class SmallGraphTest(UTestCase):
...
class InventoryEnvironmentDP(UTestCase):
    """ Inventory control """
...
class DPAgentTest(UTestCase):
    def test_agent(self):
        ...
        self.assertEqualC(agent.pi(env.reset(), 0)) # We just test the first action.
...
class DeterministicDP(UTestCase):
