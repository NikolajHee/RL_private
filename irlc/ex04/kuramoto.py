...
class ContiniousKuramotoModel(ContiniousTimeSymbolicModel): 
    def __init__(self):
        ...
        self.observation_space = Box(low=np.array([-np.inf]), high=np.array([np.inf]), dtype=float) 
        """ 
        ...
        """
        super().__init__(cost=cost, simple_bounds=None) 
...
def fk(x,u):
    """ Computes the discrete (Euler 1-step integrated) version of the Kuromoto update with discretization time dt=0.5
...
def dfk_dx(x,u):
    ...
    Look at dmodel.f for inspiration, and note it has an input argument that may be relevant.
