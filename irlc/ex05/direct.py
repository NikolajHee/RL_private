...
def direct_solver(model, options):
    ...
    for i, opt in enumerate(options):
        ...
        if i == 0 or "guess" in opt:
            # No solutions-function is given. Re-calculate by linearly interpreting bounds (see (Her21, Subsection 13.3.4))
            guess = opt['guess']
            ...
            """ For an iterative solver ((Her21, Subsection 13.3.4)), initialize the guess at iteration i to be the solution at iteration i-1.
...
def collocate(model, N=25, optimizer_options=None, guess=None, verbose=True):
    ...
    for k in range(N - 1):
        # Update cost function ((Her21, eq. (13.15))). Use the above defined symbolic expressions ts, hk and cs.
        # TODO: 2 lines missing.
        raise NotImplementedError("Update J here")
        # Set up equality constraints. See (Her21, eq. (13.18)).
    ...
    # make linear interpolant similar to (Her21, eq. (13.22))
    ...
    """ make cubic interpolant similar to (Her21, eq. (13.26)) """
...
def trapezoid_interpolant(ts, xs, fs, t_new=None):
    ''' Quadratic interpolant as in (Her21, eq. (13.26)). Inefficient but works. '''
    I = []
    ...
    implementation of  (Her21, eq. (13.26)), i.e. 
