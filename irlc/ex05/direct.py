...
def direct_solver(model, options):
    """
    Main direct solver method, see (Her23, Algorithm 21). Given a list of options of length S, the solver performers collocation
    ...
    for i, opt in enumerate(options):
        ...
        if i == 0 or "guess" in opt:
            # No solutions-function is given. Re-calculate by linearly interpreting bounds (see (Her23, Subsection 13.3.4))
            guess = opt['guess']
            ...
            """ For an iterative solver ((Her23, Subsection 13.3.4)), initialize the guess at iteration i to be the solution at iteration i-1.
...
def collocate(model, N=25, optimizer_options=None, guess=None, verbose=True):
    ...
    fs, cs = [], []  # lists of symbolic variables corresponding to f_k and c_k, see (Her23, Algorithm 20).
    for k in range(N):
    ...
    # make linear interpolant similar to (Her23, eq. (13.22))
    ...
    """ make cubic interpolant similar to (Her23, eq. (13.26)) """
...
def trapezoid_interpolant(ts, xs, fs, t_new=None):
    ''' Quadratic interpolant as in (Her23, eq. (13.26)). Inefficient but works. '''
    I = []
    ...
    implementation of  (Her23, eq. (13.26)), i.e. 
...
def get_opts(N, ftol=1e-6, guess=None, verbose=False): # helper function to instantiate options object.
    d = {'N': N,
