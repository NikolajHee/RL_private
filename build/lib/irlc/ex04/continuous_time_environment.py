...
class ContiniousTimeEnvironment(gym.Env):
    ...
    def __init__(self, discrete_model: DiscretizedModel, Tmax=None, supersample_trajectory=False, render_mode=None):
        """
        ...
        .. runblock:: pycon
            ...
            >>> import numpy as np
            >>> class MyGymSinCosEnvironment(ContiniousTimeEnvironment):
            ...     def __init__(self, Tmax=5):
            ...         discrete_model = GymSinCosPendulumModel()
        ...
        bounds = self.discrete_model.continuous_model.bounds
        # Try to guess action/observation spaces unless they are already defined.
        if self.observation_space is None:
            self.observation_space = gym.spaces.Box(low=np.asarray(self.discrete_model.continious_states2discrete_states(bounds['x_low'])),
                                                    high=np.asarray(self.discrete_model.continious_states2discrete_states(bounds['x_high'])),
                                                    dtype=float)
        if self.action_space is None:
            self.action_space = gym.spaces.Box(low=np.asarray(self.discrete_model.continious_actions2discrete_actions(bounds['u_low'])),
                                               high=np.asarray(self.discrete_model.continious_actions2discrete_actions(bounds['u_high'])),
                                               dtype=float)
    ...
    def _get_initial_state(self) -> np.ndarray:
        ...
        if self.discrete_model.continuous_model.bounds['x0_low'] == self.discrete_model.continuous_model.bounds['x0_high']:
            return np.asarray(self.discrete_model.continious_states2discrete_states(self.discrete_model.continuous_model.bounds['x0_low']))
        else:
